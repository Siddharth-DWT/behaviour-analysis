...
import { inputCls, cardCls, primaryBtnCls } from "./ui";
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleProfileSubmit} className={cardCls}>
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-nexus-text-secondary">Full Name</label>
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className={inputCls} placeholder="Your name" />
        </div>

        <div>
          <label className="mb-1 block text-xs font-medium text-nexus-text-secondary">Company</label>
          <input type="text" value={company} onChange={(e) => setCompany(e.target.value)} className={inputCls} placeholder="Your company (optional)" />
        </div>
      ...
      <form onSubmit={handlePasswordSubmit} className={cardCls}>
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-nexus-text-secondary">Current Password</label>
          <div className="relative">
            <input type={showCurrent ? "text" : "password"} value={currentPw} onChange={(e) => setCurrentPw(e.target.value)} required className={`${inputCls} pr-9`} placeholder="Current password" />
            <button type="button" onClick={() => setShowCurrent(!showCurrent)} className="absolute right-2 top-1/2 -translate-y-1/2 text-nexus-text-muted hover:text-nexus-text-secondary" tabIndex={-1}>
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-nexus-text-secondary">New Password</label>
          <div className="relative">
            <input type={showNew ? "text" : "password"} value={newPw} onChange={(e) => setNewPw(e.target.value)} required className={`${inputCls} pr-9`} placeholder="Min 8 chars, 1 uppercase, 1 number" />
            <button type="button" onClick={() => setShowNew(!showNew)} className="absolute right-2 top-1/2 -translate-y-1/2 text-nexus-text-muted hover:text-nexus-text-secondary" tabIndex={-1}>
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-nexus-text-secondary">Confirm New Password</label>
        ...
        <button type="submit" disabled={pwLoading} className={primaryBtnCls}>
