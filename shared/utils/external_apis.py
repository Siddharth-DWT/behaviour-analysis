...
Wraps the GPU-accelerated Whisper STT, Coqui TTS, and pyannote
diarization services running on the external server.
...
Environment variables:
  ...
  EXTERNAL_DIARIZE_URL   e.g. http://110.227.200.12:8008  (can share server with Whisper)
  EXTERNAL_API_KEY       API key for all services
"""
import io
import os
import json
...
def get_assemblyai_client(**kwargs) -> Optional[AssemblyAIClient]:
    """
    Get an AssemblyAIClient if the API key is configured.
    Returns None if not configured.
    """
    if not is_assemblyai_available():
        return None
    try:
...
def get_deepgram_client(**kwargs) -> Optional[DeepgramDiarizeClient]:
    """
    Get a DeepgramDiarizeClient if the API key is configured.
    Returns None if not configured.
    """
    if not is_deepgram_available():
        return None
    try:
        return DeepgramDiarizeClient(**kwargs)
    except ValueError:
        return None
