...
_VIDEO_OVERLAY_TYPES = [
    ...
    "stress_recovery",
    "tension_cluster",
    # ── Temporal patterns (T-01, T-02, T-08) missing from original list ────────
    "stress_trajectory", "engagement_decay", "escalation_ladder",
    # ── Pairwise rules missing from original list ──────────────────────────────
    "urgency_authenticity", "stress_sentiment_incongruence", "verbal_incongruence",
    "cognitive_load", "nonverbal_disagreement", "physical_engagement",
    "false_confidence", "smile_sentiment_incongruence", "processing_load",
    "dominance_anxiety", "interrupt_intent",
    # ── Hardcoded rule signals missing from original list ──────────────────────
    "momentum_shift", "persistent_incongruence",
...
async def get_video_signals(
    ...
    # The 120s duration guard filters video-agent signals that incorrectly span the full
    # session (body_mirroring, adaptation_pattern from the video pipeline — CLAUDE.md Issue 8).
    # Fusion temporal/compound patterns (behavioral_shift, escalation_ladder, stress_recovery,
    # etc.) are session-scoped by design and must NOT be filtered by this guard.
    # Interrogation panel signals also bypass — they're session-level summaries.
    _PANEL_SIGNAL_TYPES = frozenset(
        _FUSION_INTERROG_TYPES + _LANGUAGE_OVERLAY_TYPES + _CONVERSATION_INTERROG_TYPES
    )
    signals = [
        s for s in signals
        if s["agent"] != "video"  # only video-agent signals get the 120s cap
        or (s["end_ms"] - s["start_ms"]) <= 120_000
        or s["signal_type"] in _PANEL_SIGNAL_TYPES
    ]
