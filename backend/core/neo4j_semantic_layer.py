# backend/core/neo4j_semantic_layer.py
"""
...
Copied from services/api_gateway/neo4j_semantic_layer.py.
Import path change: `from neo4j_sync import` → `from core.neo4j_sync import`
"""
import json
import logging
...
# Build lookup index by name — O(1) tool dispatch by name
...
async def search_graph_context_fallback(question: str, session_id: str) -> str:
    ...
    try:
        from shared.utils.llm_client import acomplete
        from core.neo4j_sync import GRAPH_SCHEMA_HINT  # updated import path for monolith
