...
async def update_session_status(
    ...
    await pool.execute(
        f"UPDATE sessions SET {set_clause} WHERE id = $1 AND org_id = $2",
        *params,
    )
...
async def upsert_speakers(
    ...
    """Create or update speaker records. Returns {speaker_label: speaker_uuid}."""
    pool = await get_pool()
...
async def get_alerts(session_id: str, org_id: str = DEV_ORG_ID) -> list[dict]:
    ...
    rows = await pool.fetch(
        ...
        SELECT a.id, a.session_id, a.speaker_id, a.alert_type, a.severity,
               ...
               a.acknowledged, a.created_at, sp.speaker_label
        FROM alerts a
...
async def get_report(session_id: str, org_id: str = DEV_ORG_ID) -> Optional[dict]:
...
async def get_transcript(session_id: str, org_id: str = DEV_ORG_ID) -> list[dict]:
    ...
    rows = await pool.fetch(
        ...
        SELECT t.id, t.session_id, t.speaker_id, t.segment_index,
               ...
               t.sentiment, t.sentiment_score, sp.speaker_label
